...
const MESH_PARTICLE = '@/assets/3d/point.png'
...
var spScene, spCamera, spRenderer, spRendererDom

var spD1Light, spD2Light

var spSphere1Solid, spSphere1SolidFaces, spSphere1SolidVertices, spSphere1Wire
...
var spSphere2Wire, spSphere2WireVertices, spSphere2Particles
...
function spInitWorld() {
  ...
  spCamera = new THREE.PerspectiveCamera(
    40,
...
function spInitGeomSphere2() {
  ...
  import('@/assets/3d/point.png').then((res) => {
    textureLoader.load(res.default, function (texture) {
      ...
      var spSphere2WirePointMat = new THREE.PointsMaterial({
        size: 2.5,
      ...
      spSphere2Particles = new THREE.Points(
        spSphere2ParticlesPointGeom,
        spSphere2WirePointMat
      )
...
function spInitAnimate(time) {
  try {
    appState =
      document.getElementById('threeContainer').dataset.focus === 'true'
  } catch (err) {
    appState = false
  }
...
function spInitTwitch() {
  ...
  function loop() {
    ...
    TweenMax.to(tempA, 0.5, {
      y: tempA.y + THREE.Math.randFloat(-1.4, 1.4),
      ease: Power4.easeOut
    })
...
function spInitMouseMove(event) {
  ...
  if (intersects.length > 0) {
    ...
    while (i < intersects.length) {
      TweenMax.fromTo(
        spSphere1SolidFaces[intersects[i].faceIndex].color,
        0.75,
        { r: 66, g: 0, b: 198 },
        { r: 1, g: 1, b: 1 }
      )
      ...
      if (intersects[i].distance > 30) {
        ...
        TweenMax.to(tempA, 0.3, {
          y: tempA.y + THREE.Math.randFloat(-0.5, 0.5),
          ease: Expo.easeOut
        })
